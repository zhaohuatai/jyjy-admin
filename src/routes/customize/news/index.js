...
class Interlocution extends Component {
  ...
  handleRefresh = (params) => {
    ...
    params = {...params, sort: 'createTime', order: 'asc'};
  ...
  handleDelete = () => {
    Modal.confirm({
      ...
      okType: 'danger',
      onOk: () => {
        deletePubNews({id: this.state.selectedRowKeys[0]}).then(data => {
  ...
  // 更新
  handleUpdate = () => {
    loadPubNews({id: this.state.selectedRowKeys[0]}).then(data => {
      this.setState({update_data: data.data.pubNews, update_display: true})
    })
  }
  // 显示详情
  handleShowDetail = (record) => {
    loadPubNews({id: record.id}).then(data => {
      this.setState({detail_data: data.data.pubNews, detail_display: true})
    })
  }
